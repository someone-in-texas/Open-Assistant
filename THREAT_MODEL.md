...
| Information disclosure | Page requests cookies, extra tabs, hidden fields, provider tokens, secrets  | Explicit source selection, hidden/sensitive filtering, no cookie/storage API, structured context, credential isolation, redacted logs | User may intentionally paste sensitive content |
...
- A malicious or incompatible Codex process could request tools, expose credentials, or return an attacker URL. The companion pins a minimum compatible version, uses stdio rather than a listener, isolates its profile/workspace, disables execution surfaces, rejects every server tool request, emits only normalized output/status, and accepts login URLs only on credential-free OpenAI/ChatGPT HTTPS hosts. A compromised local account can still replace trusted binaries or read user-level credentials.
- A stalled or oversized native/provider response is bounded by framing, queue/concurrency/output limits and timeouts. Cancellation aborts the tracked task; native disconnect aborts remaining work and drops the Codex child.
