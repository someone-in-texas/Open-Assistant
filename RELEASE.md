...
4. The tag workflow verifies the tag/main relationship and version parity, audits dependency advisories and registry signatures, runs all gates, builds deterministic extension/source archives, submits to AMO through the approval-protected `production` environment, verifies the final signed-artifact checksums, attests the complete artifact set, and creates a draft GitHub release.
5. When AMO review completes, the status workflow attaches the Mozilla-signed XPI and publishes or updates the release. Tag builds validate the native host without publishing it; native binaries are attached only after Authenticode or Developer ID/notarization verification.
